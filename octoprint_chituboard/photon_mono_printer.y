...
class Sla_printer(Printer):
	...
	def unselect_file(self, *args, **kwargs):
		...
		self._setCurrentZ(None)
	"""
	Unsafe commands for the time being. Disabled temporarily
	"""
	def jog(self, axes, relative=True, speed=None, *args, **kwargs):
		raise NotImplementedError("Unsafe command for the moment")
	"""
	Unsafe commands for the time being. Disabled temporarily
	"""
	...
	def cancel_print(self, user=None, *args, **kwargs):
		...
		with self._comm._jobLock:
			...
			self.commands("gostop",tags=kwargs.get("tags", set()) | {"trigger:printer.cancel_print", "source:plugin", "Plugin:Chituboard"})
	...
	def pause_print(self, user=None, *args, **kwargs):
		...
		try:
			with self._comm._jobLock:
				...
				self._logger.info("paused print, sent gopause")
